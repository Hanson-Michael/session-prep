...
/// This is a plain per-sample peak scan, not True Peak — True Peak
/// (4x-oversampled, see TruePeakMeter.swift) is only measured at
/// conversion time now, for Peak Safety's gain decision, since running the
/// oversampling on every file during a folder scan was the main thing
/// slowing scans down.
...
/// a correlation coefficient as a secondary/diagnostic signal.
...
enum AudioFileAnalyzer {
    ...
    static func analyze(url: URL) -> AudioFileRecord {
        ...
        if channelCount == 1 {
            if let metrics = try? analyzeMonoChannel(file: file) {
                record.peakLeftDBFS = metrics.peakDBFS
                record.rmsLeftDBFS = metrics.rmsDBFS
            }
        ...
        do {
            ...
            record.peakLeftDBFS = analysis.left.peakDBFS
            record.peakRightDBFS = analysis.right.peakDBFS
    ...
    private static func analyzeMonoChannel(file: AVAudioFile) throws -> ChannelMetrics {
        let blockSize: AVAudioFrameCount = 65536
        guard let buffer = AVAudioPCMBuffer(pcmFormat: file.processingFormat, frameCapacity: blockSize) else {
            throw AnalyzerError.unreadable("Could not allocate buffer")
        }
        ...
        return ChannelMetrics(peakDBFS: dbfs(Double(peak)), rmsDBFS: dbfs(rms), noiseFloorDBFS: 0)
    }
