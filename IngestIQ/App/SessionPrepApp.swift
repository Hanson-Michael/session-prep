...
///   IngestIQ: About IngestIQ, Settings… (⌘,), Services,
...
///   File:   Open Folder… (⌘O), Add Files… (⇧⌘O) — replaces the default
///            "New Window" slot.
...
struct SessionPrepApp: App {
    ...
    // Needed so the Help menu's "Automatically Check for Updates" checkbox
    // actually stays visually in sync — without observing AppSettings here,
    // nothing tells the Commands menu to rebuild after the setting changes,
    // so the checkmark could get fixed at launch and never update again
    // even though the underlying value was changing correctly.
    ...
    var body: some Scene {
        ...
        .defaultSize(width: 1380, height: 760)
        .commands {
            CommandGroup(replacing: .appInfo) {
                Button("About IngestIQ") {
                    ...
                    // reentrancy — same defensive fix applied in Session
                    // Close after a debugger-only About-window crash there;
                    // harmless here regardless of whether the same race is
                    // actually present.
            ...
            CommandGroup(replacing: .newItem) {
                ...
                Button("Add Files…") {
                    NotificationCenter.default.post(name: .sessionPrepAddFiles, object: nil)
                }
